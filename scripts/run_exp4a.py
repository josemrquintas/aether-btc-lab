...
Writes to aether_btc_lab: exp4a_grid.
...
# Grid
ACTIVATION_MFE_VALUES = [0.005, 0.01, 0.02]
TRAIL_DISTANCE_VALUES = [0.003, 0.005, 0.01]
MIN_TRADES_VALID = 30
...
def ensure_table(engine) -> None:
    with engine.connect() as conn:
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS exp4a_grid (
...
def main() -> None:
    ...
    with engine.connect() as conn:
        conn.execute(text("DELETE FROM exp4a_grid WHERE run_id = :r"), {"r": "exp4a_v1"})
        conn.execute(text("""
            INSERT INTO exp4a_grid
            ...
            VALUES (:run_id, :activation_mfe, :trail_distance, :n_trades, :n_trail_exits,
                    ...
                    :calmar, :max_dd)
        """), [{"run_id": "exp4a_v1", **r} for r in results])
        conn.commit()
