#!/usr/bin/env python3
"""Experiment 5 — Trade frequency grid (relaxed asymmetric gate).
...
Writes to aether_btc_lab: exp5_grid (36 rows per run).
...
def ensure_table(engine) -> None:
    with engine.connect() as conn:
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS exp5_grid (
...
def main() -> None:
    ...
    parser.add_argument("--run-id", default="exp5_freq_v1")
    ...
    # Persist
    with engine.connect() as conn:
        conn.execute(text("DELETE FROM exp5_grid WHERE run_id = :r"), {"r": args.run_id})
        conn.execute(text("""
            INSERT INTO exp5_grid
